...
$$\\  {MURDERRATES} = \beta _{1} + \beta _{2} INCOME+ \beta _{3}  TIME + \beta _{4}  EXECUTION+ \beta _{5} CONVICTIONS+ \beta _{6}  LFP + \epsilon_{i}$$
...
$$ H_{0}: \beta_{2} - \beta_{3} \ge 0 \\H_{1}: \beta_{2} - \beta_{3} < 0$$[**Shabib: Step 3**]{.underline}
...
[**Shabib: Step 4**]{.underline} The following is a summary for Multicollinearity using VIF:
...
[**Taku: Step 5**]{.underline} To create a new model, we need to observe other regression models with packege.
...
plot(fitted_values, residuals, main="Residuals Vs Fitted", xlab="Fitted_Values", ylab="Residuals" )
...
$$\\  {MURDERRATES} = \beta _{1} + \beta _{2} INCOME+ \beta _{3}  TIME + \beta _{4}  EXECUTION+ \beta _{5} CONVICTIONS+ \beta _{6}  LFP + \beta _{7}\hat{y}^2+\epsilon_{i}$$

The RESET Hypothesis Test is as follows:

$$ H_{0}: \beta _{7} = 0 \\H_{1}: \beta _{7} \ne 0$$Running the RESET test as indicated below at a 5% significance level, we obtain the following statistics:
...
```{r}
New_Reg <- lm(rate~income+time+executions+convictions+lfp+noncauc+southern+poly(income,2), data = MurderRates)
